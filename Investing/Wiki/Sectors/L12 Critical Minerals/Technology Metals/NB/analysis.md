# NB — Analysis
*Layer 2 — Thesis, conviction, and scoring.*
*Last updated: 2026-06-15 (onboarded via /add-ticker)*

---

## One-Line Thesis
The leading US development-stage source of niobium, scandium, titanium, and magnetic rare earths (Elk Creek, Nebraska) — a financing-gated call option on reshored critical-mineral supply as China tightens rare-earth and gallium/germanium exports.

---

## Investment Thesis

> **Thesis established:** 2026-06-15
> **Last validated:** 2026-08-25
> **Drift status:** On track — Elk Creek economic assessment raised, price target increased to $12

NioCorp is developing the **Elk Creek Critical Minerals Project** in southeast Nebraska — a prospective domestic source of niobium (superalloys, capacitors, steel), scandium (aerospace alloys, solid-oxide fuel cells), titanium, and **magnetic rare earths** (neodymium, praseodymium, dysprosium, terbium) used in the permanent magnets that drive motors, robotics, and defense systems. The strategic case is reshoring: as China treats heavy-rare-earth and critical-mineral export controls as a permanent lever, a US-located, vertically integrated mine-plus-processing project carries national-security value disproportionate to its size. That has translated into a $10M Department of Defense award and an active U.S. Export-Import Bank loan application of up to ~$780M (J.P. Morgan engaged), against a ~$1.1B project capex from the 2022 feasibility study. A non-binding offtake with Traxys for the project's first ~10 years of output further de-risks the demand side.

This is, however, a **pre-revenue, pre-construction developer** — the thesis is binary and financing-gated. The entire value rests on closing the EXIM debt + equity package and executing construction on budget; until then there is no production, no revenue, and meaningful liquidity pressure (the CEO extended a personal $2M credit facility in 2026). Position it as venture-style optionality, not a cash-flowing miner.

**Key moat:** Scarcity — one of very few advanced US-located niobium/scandium/magnetic-REE projects; integrated mine + on-site processing; government and strategic-offtaker backing.

**Key risks:** Financing not yet closed (EXIM approval + large equity component); ~$1.1B capex vs. a micro-cap balance sheet → severe dilution risk; multi-year construction/permitting/execution risk; commodity price exposure (Nb/Sc/REE); going-concern/liquidity pressure pre-financing; corp domiciled in Canada (BC) though the asset is US.

---

## Scoring Summary
_Not yet scored — run `/score-ticker NB`. (Expect a low composite: the thesis is binary optionality on financing, not current fundamentals.)_

---

## Conviction Log

| Date | Event | Δ Conviction | Why |
|------|-------|-------------|-----|
| 2026-06-15 | Onboarded as the L12 critical-minerals / magnetic-REE leg (with MP) | → | Establishes coverage; EXIM financing close is the single gating catalyst. |
| 2026-02 | Began detailed engineering on the ~$45M Elk Creek mine portal | ↑ | First physical-progress step; signals intent to move toward construction ahead of full financing. |
| 2026-08-24 | Elk Creek economic assessment raised, PT increased to $12 | ↑ Strengthened | Upgraded project economics directly improves the underlying asset value the financing-gated thesis is optioned on. |
| 2026-08-26 | Railveyor automated haulage system embedded in Elk Creek mine plan | ↑ | Incremental engineering derisking of the construction plan — another physical-progress step ahead of the EXIM financing decision that gates the whole thesis. |
| 2026-09-02 | Jefferies initiates Hold, citing financing uncertainty | → | First cautionary sell-side action; names the same EXIM-financing gate the thesis is already built around rather than a new risk — confirmation, not a change in the underlying picture. |

---

## Cross-Ticker Signals

| Date | Direction | Other Ticker | Signal | Implication |
|------|-----------|-------------|--------|-------------|
| 2026-06-15 | Peer | MP | Both US rare-earth / critical-mineral plays | MP is a producing magnet-REE leader; NB is earlier-stage, broader basket (Nb/Sc/Ti) — complementary, not direct substitutes |

---

## Catalyst Timeline
- [ ] EXIM Bank loan decision (~$780M) — the gating catalyst for full financing
- [ ] Full project financing close (debt + equity) — targeted 2026 per management
- [ ] Construction start / mine-portal progress at Elk Creek
- [ ] Definitive offtake agreements (Traxys and others) converting from non-binding
- [ ] Updated feasibility / capex and any government (DoD/DOE) funding additions

---

## Analyst Coverage
- Thin small-cap coverage; refresh via `/ticker-monitor --deep NB`.

---

## Ecosystem Links
- L12 Critical Minerals & Raw Elements — Rare Earths (Neodymium, Dysprosium — motors, magnets). Materials & Mining sector; peer to MP.
