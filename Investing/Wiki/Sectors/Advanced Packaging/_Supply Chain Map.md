...
| Power Management ICs & Gate Drivers | Design and supply the analog power-conversion silicon that regulates, sequences, and delivers voltage to every digital load — the binding constraint on AI compute-node power density | Multi-phase buck/boost controller design; BCD (Bipolar-CMOS-DMOS) process integration of power FETs + analog control on one die; vertical power module (VPM) co-packaging of inductor + power stage beneath the processor; gate-driver level-shifting for GaN/SiC switches; 800V→48V→core-rail conversion topology design; load-line/telemetry calibration; automotive AEC-Q100 qualification | Multi-phase PMIC / VR controller (GPU/CPU core rails); vertical power module (VPM, point-of-load under the die); DrMOS power stage; GaN/SiC gate driver; PoL DC-DC converter; battery-management & sequencing IC | Partial | Low–Medium (fabless) | Process IP + Design-win lock-in (proprietary BCD process; scarce analog design talent) | High |
