# AVAV — Analysis
*Layer 2 — Thesis, conviction, and scoring.*
*Last updated: 2026-06-15 (onboarded via /add-ticker)*

## One-Line Thesis
The US military's primary small-drone and loitering-munitions supplier (Puma, Raven, Switchblade), now a broader multi-domain defense-tech platform post-BlueHalo — the go-to public vehicle for battlefield-proven autonomous-drone exposure.

## Investment Thesis

> **Thesis established:** 2026-06-15 / **Last validated:** 2026-09-02 / **Drift status:** On track, strengthening — $465M Army laser weapons contract confirms directed energy as a funded, revenue-generating leg of the multi-domain platform, alongside continued core-product order flow

AeroVironment is the entrenched Western incumbent in small unmanned aircraft systems and loitering munitions. Its Puma/Raven/Wasp/JUMP 20 sUAS and the Switchblade family are embedded in US and allied programs of record and have been combat-validated at scale in Ukraine — incumbency, ITAR/classified barriers, and battlefield reputation are the moat. The 2025 BlueHalo combination transformed AV from a drone manufacturer into a multi-domain platform spanning space, counter-UAS, electronic warfare, and directed energy, enlarging the addressable defense budget it can capture.

The thesis is leveraged to a structural rearmament cycle: drone-centric warfare, attritable autonomous systems, and counter-UAS are among the fastest-growing defense priorities, and AV is the clearest pure-play public name. As an edge-robotics story it fits the Edge & Physical AI surface's "Drones and UAVs" box and anchors a new "UAV & Drone Systems" tier. The offsets are classic defense-name risks: revenue lumpiness tied to government budgets and continuing resolutions, program-timing volatility, integration of BlueHalo, and intensifying competition from low-cost drone entrants (including loitering-munition startups).

**Key moat:** Programs of record + battlefield-proven incumbency + ITAR/classified barriers (made-in-USA defense supplier).
**Key risks:** Budget/CR-driven lumpiness; BlueHalo integration; low-cost-drone competition; valuation sensitivity to conflict-demand expectations.

## Scoring Summary
_Not yet scored — run `/score-ticker AVAV`._

## Conviction Log

| Date | Event | Δ Conviction | Why |
|------|-------|--------------|-----|
| 2026-06-15 | Onboarded to wiki | New | Closes the Edge-surface "Drones and UAVs" box; anchors new UAV & Drone Systems tier; structural defense-drone cycle. |
| 2026-08-25 | $30.9M German Puma UAS contract win | ↑ Strengthened | Direct, named order flow on the core Puma product line confirms continued allied demand post-BlueHalo. |
| 2026-08-26 | $80.5M Titan MS order; Switchblade 600 fielded across 20+ Army brigade combat teams; CEO cites "unprecedented" demand outlook | ↑ Strengthened | New order flow plus broad Army adoption of the loitering-munitions line and management's own multi-year demand framing reinforce the core-product thesis. |
| 2026-08-27 | $51M Army order for Switchblade systems | ↑ Strengthened | Continues confirmed order-flow pattern on the core product line. |
| 2026-08-29 | Greek expansion + record backlog; NASA taps AVAV for three Mars helicopters (2028 mission) | ↑ Strengthened | Record backlog confirms demand independent of the stock's price action; the NASA contract is a new, non-defense revenue line that broadens the "multi-domain platform" thesis beyond military drones. |
| 2026-09-02/03 | $465M Army contract for laser weapons | ↑ Strengthened | Converts the 08-30 CUAS laser test demo into a funded program — directed energy is now a confirmed, funded revenue leg of the post-BlueHalo platform, not just a capability demo; largest single order logged to date. |

## Cross-Ticker Signals

| Date | Counterparty | Signal | Direction |
|------|--------------|--------|-----------|
| 2026-06-15 | RKLB (Space & Comms) | Adjacent defense/space-tech buildout | Parallel |
| 2026-06-15 | JOBY / ACHR | Broader eVTOL/UAV airframe ecosystem (rendered as bare chips) | Parallel |

## Catalyst Timeline
- [ ] Next earnings — backlog + book-to-bill; Switchblade order flow
- [ ] BlueHalo integration + cross-sell milestones
- [ ] US defense budget / supplemental funding outcomes

## Analyst Coverage
Constructive on the drone-warfare/rearmament cycle; watch valuation vs. budget risk. Refresh PT via /ticker-monitor --deep AVAV.

## Ecosystem Links
- **AI Buildout Stack:** Edge & Physical AI surface — "Drones and UAVs."
- DATA tier: Robotics & Edge AI → "UAV & Drone Systems" (new). Co-chips: JOBY, ACHR (bare chips).
