...
| Autonomous Vehicles & ADAS | Productizes the full self-driving / driver-assist stack for road vehicles — perception, HD mapping, planning, and safety policy delivered to automakers | EyeQ system-on-chip vision processing (multi-camera + radar + LIDAR sensor fusion), REM (Road Experience Management) crowdsourced HD-map harvesting from production fleets, RSS (Responsibility-Sensitive Safety) formal driving-policy modeling, ASIL-D functional-safety silicon + software co-design, surround camera/radar reference-design validation, ADAS feature OTA rollout (L2+ → L4) | EyeQ6 ADAS SoC, SuperVision (eyes-on hands-off L2+), Chauffeur (eyes-off L3/L4) system, REM HD map, automotive FMCW LIDAR, AV domain controller, robotaxi drive unit | Partial | Medium–High | Process IP + Data network effects (fleet-sourced maps) + ASIL-D qualification | Medium–High |
| UAV & Drone Systems | Designs and fields complete unmanned aircraft + loitering-munition systems integrating airframe, autonomy, datalink, and payload | Composite airframe design + layup, flight-control autopilot tuning (GPS-denied visual-inertial navigation), encrypted C2 datalink integration, EO/IR gimbal payload calibration, swarming/autonomy mission-software development, MIL-STD environmental + EMI qualification, loitering-munition terminal-guidance integration | Small UAS (Raven, Puma, JUMP 20), loitering munition (Switchblade 300/600), Group 1–3 drone, eVTOL air taxi, encrypted ground control station (GCS), counter-UAS system | Partial | Medium–High | Systems engineering + DoD program-of-record lock-in + autonomy software | Medium |
