---
# Layer 1 — Facts KB (machine-readable)
ticker: SNDK
company: "SanDisk Corporation"
cik: "0001110803"
exchange: NASDAQ
sector: Semiconductors

management:
  - role: CEO
    name: "David Goeckeler"
    ownership_pct: null
    notes: "Led the February 2025 separation of SanDisk from Western Digital; prior WDC CEO."
  - role: CFO
    name: "Luis Visoso"
    ownership_pct: null
    notes: "Widely praised for capital discipline and balance sheet management."
  - role: "President, Technology & Strategy"
    name: "Siva Sivaram"
    ownership_pct: null
    notes: "Flash technology architect."

earnings:
  - period: "Q3 FY2026"
    date: "2026-04-30"
    period_end: "2026-04-03"
    revenue_m: 5950
    gross_margin_pct: 78.4
    eps_nongaap: 23.41
    eps_gaap: 23.03
    net_income_m: 3615
    free_cash_flow_m: 2955
    datacenter_revenue_m: 1467
    datacenter_pct: 24.7
    beat: true
    guidance_prev_revenue_m_low: 4400
    guidance_prev_revenue_m_high: 4800
    guidance_prev_eps_low: 12.00
    guidance_prev_eps_high: 14.00
    notes: "Revenue $5.95B crushed guidance of $4.4–4.8B; +251% YoY, +97% QoQ. EPS $23.41 vs $12–14 guided. Datacenter +233% QoQ; over 1/3 of FY2027 bit supply contracted ($11B+ enforceable guarantees). 10-Q filed 2026-05-01."
  - period: "Q4 FY2026 (guidance)"
    date: "2026-04-30"
    revenue_m_low: 7750
    revenue_m_high: 8250
    eps_nongaap_low: 30.00
    eps_nongaap_high: 33.00
    notes: "Q4 FY2026 outlook issued with Q3 results. 128TB enterprise SSD expected to begin shipping this quarter."
  - period: "Q4 FY2026"
    date: "2026-04-30"
    revenue_m: null
    gross_margin_pct: 78.4
    operating_margin_pct: 69.98
    net_income_m: null
    datacenter_revenue_m: 1467
    datacenter_pct: 24.7
    notes: "Datacenter +645% YoY from $197M (Q3 FY2025). 128TB enterprise SSD not yet shipping. Q4 guided $7.75–8.25B."
  - period: "TTM (as of Q4 FY2026)"
    date: "2026-04-30"
    revenue_m: 13180
    gross_margin_pct: 78.4
    operating_margin_pct: 69.98
    net_income_m: 4510
    cash_m: 3740
    long_term_debt_m: 0
    notes: "TTM revenue +251% YoY. Zero long-term debt (down from $1.83B). $6B share buyback authorized."

filings:
  - type: "10-Q"
    period: "Q3 FY2026"
    period_end: "2026-04-03"
    date: "2026-05-01"
    url: "https://www.sec.gov/Archives/edgar/data/0002023554/000162828026029401/sndk-20260403.htm"
    notes: "Q3 FY2026 10-Q. CEO quote: 'fundamental inflection point — deliberate shift toward Datacenter.'"
  - type: "8-K"
    period: "Q3 FY2026 earnings"
    date: "2026-04-30"
    url: "https://www.sec.gov/Archives/edgar/data/0002023554/000162828026028879/sndkq3-26ex991xpressrelease.htm"
    notes: "Q3 FY2026 press release. Revenue $5.95B, Non-GAAP EPS $23.41; Q4 guidance $7.75–8.25B."
  - type: "10-Q"
    period: "Q2 FY2026"
    period_end: "2026-01-02"
    date: "2026-01-30"
    url: "https://www.sec.gov/Archives/edgar/data/0002023554/000162828026004407/sndk-20260102.htm"
    notes: "Q2 FY2026 10-Q. Datacenter $269M in Q1 FY2026; Stargate AI storage product qualifications underway."
  - type: "10-Q"
    date: "2026-04-30"
    notes: "Q3 FY2026 (fiscal) / Q4 earnings call. CEO quote: 'fundamental inflection point — deliberate shift toward Datacenter.'"

moat:
  type: "Scale + Process IP (NAND)"
  pricing_power: high
  competition_intensity: high
  made_in_usa: partial
  notes: "Pure-play NAND/flash after the Feb-2025 WDC spinoff; NAND fab joint venture with Kioxia (Japan) extended through 2034 ($1.17B guaranteed manufacturing payments). NBM (New Business Model): multi-year contracts with 5 hyperscalers, $42B contracted minimum revenue — supply priority for signatories breaks the 30-year commodity cycle. HBF (High-Bandwidth Flash) is the disruptive optionality. 78.4% gross margins at peak cycle (Apr 2026)."

tech_exposure:
  - technology: "NAND Flash"
    exposure: primary
    notes: "~100% revenue from NAND; enterprise + client SSD; BiCS8 218-layer, 2Tb QLC."
  - technology: "HBF (High-Bandwidth Flash)"
    exposure: primary
    notes: "Pioneer; 1.6 TB/s read at 8-16x HBM capacity. TAB Jul 2025; SK Hynix standardization MOU Aug 2025; samples H2 2026, products 2027-2028."
  - technology: "Memory & Storage for Connectomics & Artificial-Brain Workloads"
    exposure: primary
    notes: "Capacity-at-bandwidth bridge (HBF) + QLC archive — see Technology Preferences Race 4."

metrics:
  score: null
  score_label: "—"
  last_scored: null
  valuation_fpe: 32.3
  valuation_fpe_date: "2026-06-16"
  valuation_fpe_fy2027: 11.7
  price_at_fpe: 2107.86
  market_cap_b: 312.15
  analyst_pt: 1751
  analyst_pt_high: 3250
  analyst_pt_low: 1000
  analyst_upside_pct: -16.9
  analyst_count: 22
  analyst_buy: 18
  analyst_hold: 3
  analyst_sell: 1
  analyst_notes: "Consensus Buy (22 analysts). Citi PT $2,025 (highest major bank). Morgan Stanley upgrade. Goldman Sachs: memory cheapest AI bucket. FY2027 consensus revenue $45B (vs. $20B FY2026)."

last_updated: "2026-06-17"
next_earnings: "2026-07-30"
---
