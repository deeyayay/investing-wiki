# AAOI — Applied Optoelectronics
*Last updated: May 11, 2026*

---

## One-Line Thesis
High-beta optical interconnect pure play catching its first real 1.6T production orders — highest upside in the sector if execution holds, but meaningful execution risk remains.

## Investment Thesis

> **Thesis established:** 2026-05-11
> **Last validated:** 2026-05-12
> **Drift status:** On track — first 800G volume shipments confirmed Q1 2026; 1.6T ramp in progress

AAOI makes optical transceivers and subsystems for data centers, cable TV, and telecom. The stock led all photonics names with +328% YTD through April 2026 — the market is pricing in AAOI's transition from secondary supplier to meaningful 1.6T volume player. As a smaller, more concentrated pure-play than COHR or LITE, AAOI has higher beta to the optical theme in both directions. The key question is whether they can sustain execution and manufacturing quality as order volumes scale.

**Key moat:** Pure-play optical transceiver exposure, vertically integrated manufacturing (in-house epitaxial wafer growth), low-cost production via Malaysian facilities.

**Key risks:** Execution risk vs. incumbent suppliers (COHR, LITE), customer concentration, commoditization risk, stock already up 3x+ (valuation risk), smaller balance sheet limits capacity investment.

---

## Management & Leadership
*(Key executives, track record, alignment signals)*

| Role | Name | Notes |
|------|------|-------|
| CEO | Chih-Hsiang (Thompson) Lin, Ph.D. | Founder; CEO since inception (1997); Chairman since Jan 2014; 29+ year tenure; former research scientist at Univ. of Houston |
| CFO | Stefan J. Murry, Ph.D. | CFO since Aug 2014; company lifer since 1997; prior roles in sales, marketing, and engineering; EE Ph.D. from Univ. of Houston |

**Execution track record:** Founder-led with a stable, long-tenured management team (avg. ~13.5 years); navigated multiple optical downturns and led the successful pivot to 400G/800G/1.6T data center transceivers — first 800G volume shipments to a large hyperscaler confirmed in Q1 2026, validating the transition thesis.

**Insider ownership / alignment:** CEO Thompson Lin holds ~3.3M shares (largest individual insider position; exact % not confirmed from DEF 14A — EDGAR returned 403). 2025 proxy (filed Apr 28, 2025) shows ~90% of CEO target comp is variable, tied to revenue, non-GAAP operating income, design wins, and 3-year relative/absolute TSR — strong pay-for-performance alignment.

---

## Catalyst Timeline
- [ ] 1.6T order ramp — first orders received, monitor volume milestones
- [ ] Gross margin improvement as mix shifts to higher-value 800G/1.6T
- [ ] New hyperscaler customer announcements
- [ ] Quarterly earnings — revenue cadence and guidance

---

## Conviction Log
<!-- Append-only. One entry per meaningful event that shifts or confirms your view. -->
<!-- Δ Conviction: ↑ Strengthened / ↓ Weakened / → Neutral -->

| Date | Event | Δ Conviction | Why |
|------|-------|-------------|-----|
| 2026-05-12 | Q1 2026 earnings: first 800G volume shipment to large hyperscaler confirmed; FY2026 consensus raised to $1.03B (+103% YoY) | ↑ Strengthened | Execution risk was the primary bear case — volume shipment at scale to a hyperscaler is the single most important data point to validate the thesis that AAOI can compete with COHR/LITE at volume |

---

## Earnings & Financials
*(Append after each earnings event)*

| Date | Revenue | Beat/Miss | Guidance | Notes |
|------|---------|-----------|----------|-------|
| Q1 2026 (May 12) | Record Q1 | EPS miss ($-0.07 vs $-0.06 est) | Q2 $150M–$165M | First 800G volume shipment to large hyperscaler; analysts raised PT +48% to $151 |

---

## News & Alpha Log
*(Append notable developments: partnerships, product launches, supply chain signals)*

- **2026-05-11** — Note initialized. Sector YTD leader at +328% through April 22. First 1.6T production orders received — primary thesis validator.

---

## SEC Filings
*(Index of filing notes in this folder)*

| Filing | Date | File |
|--------|------|------|
| — | — | — |

---

## Cross-Ticker Signals
<!-- Log signals this ticker emits to or receives from other monitored names. -->
<!-- Direction: "Receives" = another ticker's news has implications here. "Emits" = AAOI's news signals something for others. -->

| Date | Direction | Other Ticker | Signal | Implication |
|------|-----------|-------------|--------|-------------|
| 2026-05-12 | Receives | NVDA | NVIDIA Blackwell demand guidance raised; hyperscaler capex expanding | As a 1.6T transceiver supplier, AAOI is a direct downstream beneficiary of any NVIDIA-driven cluster build — higher GPU density = more optical ports per rack |

---

## Analyst Coverage
*(Rating changes, price target moves, notable commentary)*

- **2026-05-11** — No analyst notes yet.

---

## Research Log
- **2026-05-11** — Note migrated to subfolder. Removed Technicals and Options sections. Knowledge base format going forward.

- **2026-05-12** — 🔥 EARNINGS | AAOI Q1 2026: EPS -$0.07 (missed -$0.06), record Q1 revenue. First 800G volume shipment to large hyperscale customer. Analysts raised PT +48% to $151; FY2026 consensus revenue now $1.03B (+103% YoY). Q2 guided $150M–$165M. Source: SEC/SimplyWallSt. [[AAOI_8K_2026-05-07]]
- **2026-05-14** — stock-research run. Fundamental sections populated from web research and SEC DEF 14A.

## Social Mentions

| Date | Signal | Source |
|------|--------|--------|
| 2026-05-17 | [[2026-05-17-sive-when-i-see]] | tweet |
| 2026-05-20 | [[2026-05-20-nvda-q1-earnings-preview]] | tweet |
